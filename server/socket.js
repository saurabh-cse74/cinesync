...
const db = require('./db');
const { assertSafeUrl } = require('./security');
...
module.exports = function attach(io, sessionMiddleware) {
  ...
  io.on('connection', socket => {
    ...
    const sys = (text, target = code) => io.to(target).emit('chat:message', { system: true, text, at: Date.now() });
    ...
    socket.on('room:join', async (data, ack) => {
      ...
      if (code && code !== meta.room_code) {
        const previous = R.get(code);
        socket.leave(code);
        socket.leave(`${code}:${socket.user.id}`);
        if (previous) {
          const old = previous.participants.get(socket.user.id);
          old?.sockets.delete(socket.id);
          if (old && old.sockets.size === 0) previous.participants.delete(socket.user.id);
          io.to(code).emit('room:state', R.snapshot(previous));
        }
        code = null;
      }
      const targetCode = meta.room_code;
      const r = R.ensure(targetCode, {
      ...
      socket.join(code);
      socket.join(`${code}:${socket.user.id}`);
    ...
    socket.on('media:change', async payload => {
      const roomCode = code, r = room(); if (!r) return;
      if (!R.canControl(r, socket.user.id)) return socket.emit('room:error', 'Only the host can change the video.');
      if (!payload || typeof payload !== 'object') return socket.emit('room:error', 'Invalid media URL.');
      const m = { type: String(payload.type || 'url'), title: String(payload.title || 'Video').slice(0, 200), url: String(payload.url || '') };
      const validMedia = (m.type === 'youtube' && /^[A-Za-z0-9_-]{11}$/.test(m.url)) ||
        (['url', 'demo'].includes(m.type) && /^https:\/\//i.test(m.url)) ||
        (m.type === 'uploaded' && /^\/media\/[a-f0-9]{32}\.(mp4|webm|mov|m4v)$/i.test(m.url));
      if (!validMedia) return socket.emit('room:error', 'Invalid media URL.');
      if (m.type === 'url' || m.type === 'demo') {
        try {
          const safeUrl = await assertSafeUrl(m.url);
          if (!/\.(mp4|webm|mov|m4v)$/i.test(safeUrl.pathname))
            return socket.emit('room:error', 'This video URL has an unsupported file type.');
          m.url = safeUrl.href;
        } catch (err) {
          return socket.emit('room:error', err.message);
        }
      }
      if (code !== roomCode || R.get(roomCode) !== r) return;
      r.media = m;
      r.playback = { isPlaying: false, currentTime: 0, playbackRate: 1, updatedAt: Date.now() };
      const meta = await db.getRoom(roomCode); if (meta) db.saveMedia(meta.id, m);
      io.to(roomCode).emit('room:state', R.snapshot(r));
      sys(`Video changed to "${m.title}"`, roomCode);
    });
    ...
    socket.on('chat:send', async text => {
      const roomCode = code, r = room(); if (!r || !r.chatEnabled) return;
      const clean = String(text || '').trim().slice(0, 1000);
      if (!clean) return;
      const meta = await db.getRoom(roomCode); if (meta) db.saveMessage(meta.id, socket.user.id, clean);
      if (code === roomCode && R.get(roomCode) === r)
        io.to(roomCode).emit('chat:message', { userId: socket.user.id, name: socket.user.name, text: clean, at: Date.now() });
    });
    ...
    ['webrtc:offer', 'webrtc:answer', 'webrtc:ice', 'webrtc:hangup', 'webrtc:renegotiate'].forEach(ev =>
      socket.on(ev, data => {
        if (!code) return;
        const payload = data && typeof data === 'object' ? data : { data };
        const targets = payload.to ? (Array.isArray(payload.to) ? payload.to : [payload.to]) : null;
        const message = { from: socket.user.id, data: payload.data ?? payload };
        if (targets && targets.length) {
          targets.forEach(userId => socket.to(`${code}:${userId}`).emit(ev, message));
          return;
        }
        socket.to(code).emit(ev, message);
      }));
