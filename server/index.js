...
const DEFAULT_UPLOAD_DIR = path.join(require('os').tmpdir(), 'cinesync-uploads');
let UPLOAD_DIR = process.env.UPLOAD_DIR || DEFAULT_UPLOAD_DIR;
const MAX_MB = Number(process.env.MAX_UPLOAD_SIZE_MB || 2048);

try {
  fs.mkdirSync(UPLOAD_DIR, { recursive: true });
} catch (err) {
  const fallbackDir = DEFAULT_UPLOAD_DIR;
  try {
    fs.mkdirSync(fallbackDir, { recursive: true });
    UPLOAD_DIR = fallbackDir;
    console.warn(`[upload] ${process.env.UPLOAD_DIR || 'default'} is not writable; using ${UPLOAD_DIR} instead.`);
  } catch (fallbackErr) {
    const localFallbackDir = path.join(process.cwd(), '.cinesync-uploads');
    fs.mkdirSync(localFallbackDir, { recursive: true });
    UPLOAD_DIR = localFallbackDir;
    console.warn(`[upload] Falling back to ${UPLOAD_DIR} because the configured upload directory is unavailable.`);
  }
}
...
// ---------- Auth ----------
// Allow direct name-based entry for the watch room experience.
...
app.get('/media/:file', requireAuth, (req, res) => {
  ...
  if (!match[1]) {
    ...
    if (!suffix) return res.status(416).set('Content-Range', `bytes */${size}`).end();
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`CineSync on http://0.0.0.0:${port}`);
});
