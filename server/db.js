...
async function createRoom(r) {
  if (!pool) {
    ...
    mem.rooms.set(r.room_code, { ...r, created_at, expires_at: new Date(created_at.getTime() + 24 * 60 * 60 * 1000), is_active: true });
    return mem.rooms.get(r.room_code);
  }
...
async function getRoom(code) {
  if (!pool) {
    ...
    return room?.is_active && room.expires_at > new Date() ? room : null;
  }
...
async function recentRooms(userId) {
  if (!pool) return [...mem.rooms.values()].filter(r => r.is_active && r.expires_at > new Date() &&
    (r.host_id === userId || (mem.members.get(r.id) || new Set()).has(userId)))
  ...
  const { rows } = await q(`SELECT r.* FROM rooms r LEFT JOIN room_members m ON m.room_id=r.id
    WHERE (r.host_id=$1 OR m.user_id=$1) AND r.is_active AND r.expires_at > now()
    GROUP BY r.id ORDER BY r.created_at DESC LIMIT 10`, [userId]);
  return rows;
}
