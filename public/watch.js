...
function connect() {
  ...
  socket.on('peer:joined', ({ userId }) => {
    ...
    if (String(me.id) < String(userId)) makeOffer(userId);
  });
  socket.on('peer:left', ({ userId }) => { if (userId) endCall(userId); });
  socket.on('webrtc:offer', ({ from, data }) => onOffer(from, data));
  socket.on('webrtc:answer', async ({ from, data }) => {
    const pc = peerConnections[from]; if (!pc) return;
    try { await pc.setRemoteDescription(data); if (pc.needsNegotiation) makeOffer(from); }
    catch (err) { console.error('Could not apply call answer', err); }
  });
  socket.on('webrtc:renegotiate', ({ from }) => {
    const pc = peerConnections[from]; if (!pc) return;
    pc.needsNegotiation = true; makeOffer(from);
  });
...
function onState(s) {
  ...
  s.participants.forEach((p, i) => {
    if (i) people.append(document.createElement('br'));
    people.append(document.createTextNode(`${p.online ? '🟢' : '⚪'} ${p.name}`));
    if (p.id === s.hostId) {
      const host = document.createElement('span');
      host.className = 'dim'; host.textContent = ' (host)'; people.append(host);
    }
  });
...
function skipVideo(seconds) {
  ...
  v.currentTime = Number.isFinite(v.duration) ? Math.max(0, Math.min(v.duration, target)) : Math.max(0, target);
}
$('prev').onclick = () => skipVideo(-10);
$('next').onclick = () => skipVideo(10);
v.addEventListener('timeupdate', () => {
  ...
  $('pp').title = v.paused ? 'Play' : 'Pause';
  $('pp').setAttribute('aria-label', $('pp').title);
});
v.addEventListener('play', () => { $('pp').textContent = '❚❚'; $('pp').title = 'Pause'; $('pp').setAttribute('aria-label', 'Pause'); });
v.addEventListener('pause', () => { $('pp').textContent = '▶'; $('pp').title = 'Play'; $('pp').setAttribute('aria-label', 'Play'); });
...
async function shareScreen() {
  ...
  if (!navigator.mediaDevices?.getDisplayMedia) {
    sysMsg('Screen sharing is unavailable in this browser or page context.'); return;
  }
  try {
    screenStream = await navigator.mediaDevices.getDisplayMedia({ video: true });
    const track = screenStream.getVideoTracks()[0];
    if (!track) { stopScreenShare(); return; }
    track.addEventListener('ended', stopScreenShare, { once: true });
    selfcam.srcObject = screenStream;
    selfcam.style.display = 'block';
    selfcam.style.transform = 'none';
    $('screenbtn').textContent = '⏹ Stop sharing';
    Object.values(peerConnections).forEach(addLocalTracks);
    sysMsg('You are sharing your screen with people in this call.');
  } catch (err) {
    screenStream = null;
    if (err.name !== 'NotAllowedError') sysMsg('Unable to start screen sharing. Check your browser permissions.');
  }
}
...
function stopScreenShare() {
  ...
  selfcam.style.transform = 'scaleX(-1)';
  if (local) selfcam.srcObject = local;
  else { selfcam.srcObject = null; selfcam.style.display = 'none'; }
  Object.values(peerConnections).forEach(addLocalTracks);
...
function renderMsg(m) {
  ...
  if (m.system) d.textContent = m.text;
    ...
    header.textContent = `${m.name} · ${new Date(m.at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
    d.append(header, document.createTextNode(m.text));
  }
...
// ------------------------------------------------------------- WebRTC
let local = null, screenStream = null;

function renderRemoteTiles() {
  const ids = Object.keys(peerStreams);
  Object.keys(remoteTiles).forEach(id => {
    if (!peerStreams[id]) { remoteTiles[id].remove(); delete remoteTiles[id]; }
  });
  ids.forEach(id => {
    const peer = (state?.participants || []).find(p => p.id === id);
    let tile = remoteTiles[id];
    if (!tile) {
      tile = document.createElement('div'); tile.className = 'remote-tile';
      ...
      tile.append(video, label); remotes.append(tile); remoteTiles[id] = tile;
    }
    const video = tile.querySelector('video');
    if (video.srcObject !== peerStreams[id]) video.srcObject = peerStreams[id];
    tile.querySelector('.lbl').textContent = peer?.name || 'Friend';
  });
}

function setActiveRemote(userId) {
  if (userId && peerStreams[userId]) renderRemoteTiles();
}

function addLocalTracks(pc) {
  const senders = pc.getSenders();
  const videoTrack = (screenStream || local)?.getVideoTracks()[0] || null;
  let changed = false;
  (local?.getAudioTracks() || []).forEach(track => {
    if (!senders.some(sender => sender.track === track)) { pc.addTrack(track, local); changed = true; }
  });
  const videoSender = senders.find(sender => sender.track?.kind === 'video');
  if (videoSender && videoSender.track !== videoTrack) {
    videoSender.replaceTrack(videoTrack).catch(err => console.error('Could not switch shared video', err));
  } else if (!videoSender && videoTrack) {
    pc.addTrack(videoTrack, screenStream || local); changed = true;
  }
  if (changed && pc.remoteDescription) {
    const userId = Object.keys(peerConnections).find(id => peerConnections[id] === pc);
    if (userId && String(me.id) < String(userId)) { pc.needsNegotiation = true; makeOffer(userId); }
    else if (userId) socket.emit('webrtc:renegotiate', { to: userId });
  }
}
...
function ensurePeerConnection(userId) {
  ...
  pc.ontrack = e => {
    const stream = e.streams[0] || peerStreams[userId] || new MediaStream();
...
$('callbtn').onclick = async () => {
  ...
  if (local) {
    selfcam.srcObject = screenStream || local;
    selfcam.style.display = 'block';
    selfcam.style.transform = screenStream ? 'none' : 'scaleX(-1)';
  ...
  (state?.participants || []).filter(p => p.id !== me.id).forEach(p => {
    ...
    if (String(me.id) < String(p.id)) makeOffer(p.id);
  });
};
...
async function makeOffer(userId) {
  ...
  } catch (err) { console.error('Could not start call with peer', err); }
}
...
async function onOffer(from, offer) {
  ...
  } catch (err) { console.error('Could not answer call from peer', err); }
}
